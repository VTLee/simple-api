export TEMP_CREDENTIALS=$(aws sts assume-role --role-arn arn:aws:iam::a:role/cicd --role-session-name $CI_PIPELINE_ID)
...
sam deploy --template-file sam-api-template.yml --stack-name API-$API_NAME --capabilities CAPABILITY_IAM  --parameter-overrides $(cat authentication.poc.env) --no-fail-on-empty-changeset
