...
aws s3 cp dist.zip s3://$S3_BUCKET/${S3_KEY} --quiet
...
sam deploy --template-file sam-ddb-template.yml --stack-name DDB-table-stack --no-fail-on-empty-changeset
echo "Deploting API Template"
cat dist.env
sam deploy --template-file sam-api-template.yml --stack-name API-$API_NAME --capabilities CAPABILITY_IAM  --parameter-overrides $(cat dist.env) --no-fail-on-empty-changeset
